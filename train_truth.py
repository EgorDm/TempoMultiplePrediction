...
def main(dataset, name):
    ...
    ys = np.array(list(map(lambda v: to_categorical(int(math.log2(v) if v > 0 else 0), 4), samples[:, 2])))
    ...
    print('Train...')
    checkpoint = ModelCheckpoint(f'{constants.SAVE_PATH}/{name}_weights-{{epoch:02d}}-{{val_loss:.2f}}.h5', verbose=2, monitor='val_loss', save_best_only=True, mode='auto')
    model.fit(x=xs, y=ys, batch_size=200, epochs=30, verbose=1, validation_split=0.04, shuffle=True, callbacks=[checkpoint])
